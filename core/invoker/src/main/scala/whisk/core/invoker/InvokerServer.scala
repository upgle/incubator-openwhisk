            ...
            if(username == invokerUsername && password == invokerPassword){
              Invoker.messageProducer match {
                case Some(producer) => {
                  ...
                  producer.send("health", PingMessage(InvokerInstanceId(-1 - Invoker.currentInstance.get.instance, Invoker.currentInstance.get.uniqueName)), 3).andThen {
                    case Success(_) =>
                      producer.close();
              ...
              Invoker.messageProducer match {
                ...
                case None =>{
                  ...
                  Scheduler.scheduleWaitAtMost(1.seconds)(() => {
                    healthProducer.send("health", PingMessage(InvokerInstanceId(Invoker.currentInstance.get.instance, Invoker.currentInstance.get.uniqueName))).andThen {
