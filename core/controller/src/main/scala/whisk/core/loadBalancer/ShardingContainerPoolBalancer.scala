...
class ShardingContainerPoolBalancer(config: WhiskConfig, controllerInstance: InstanceId)(
  ...
  private val activationsPerController = TrieMap[InstanceId, LongAdder]()
  private val activationsPerInvoker = TrieMap[InstanceId, LongAdder]()
  ...
  override def activeActivationsByController(controller: String): Future[Int] = Future.successful(activationsPerController.get(InstanceId(controller.toInt)).map(_.intValue()).getOrElse(0))
  override def activeActivationsByInvoker(invoker: String): Future[Int] = Future.successful(activationsPerInvoker.get(InstanceId(invoker.toInt, Some(invoker))).map(_.intValue()).getOrElse(0))
...
case class ActivationEntry(id: ActivationId,
                           ...
                           controllerName: InstanceId=InstanceId(0))
